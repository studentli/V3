...
function cate($var){ 
 ...
 function cartpath($var){
 	$t=M("shop_project")->where(array('id'=>$var))->find();
 	return $t['imagepath'];
 }
...
function sfjhff($r) {
 ...
 function user_jj_lx($var){
	...
	// added by skyrim
 	...
 	if($diff>$settings['out_day_diff']){
 		$diff =$settings['out_day_diff'];
 	}
	// added by skyrim
 	// purpose: custom interest rate
 	// version: v10.0
...
if($tgbz_user_xx['sfjl']==1){
 ...
 function jlj4($a,$b){
 	$tgbz_user_xx=M('user')->where(array('UE_account'=>$a))->find();
 	if(!$tgbz_user_xx['commended']){
		M('user')->where(array('UE_account' => $a))->setInc('tj_he1',$b);
		M('user')->where(array('UE_account' => $a))->setInc('commended');
	}
 	return $tgbz_user_xx['ue_accname'];
 }
// added by skytim
...
function jlsja($var,$total_team){
 ...
 // added ends
	...
	function up_quota($uname){
		//提升投资额度
		$settings = include( APP_PATH . 'Home/Conf/settings.php' );
		$model = M("user");
		$up_num = $model->field("quota")->where(array("UE_account"=>$uname))->find();
		
		$add = empty($up_num['quota'])?$settings['supply_money_lower_limit']:0;
		$quota = ($up_num['quota']>=$settings['quota_limit'])?$settings['quota2']:$quota = $settings['quota1'];
		
		if($up_num['quota']>=$settings['supply_money_upper_limit']){
			return;
		}
		$model->where(array("UE_account"=>$uname))->setInc('quota',$quota+$add);
	}
 
  function getTreeInfo($id,$total_team,$sum=0) {
 ...
 // purpose: for different share
 // version: 1.0
 jlj4($tgbz_user_xx['ue_accname'],$ppddxx['jb']*floatval($settings['tjr_share']));//推荐奖
 // added ends
 // deleted ends
 // added by skrim
 // purpose: for different share
...
// purpose: calc masses share
// version: 5.0
$this_node = $tgbz_user_xx['ue_accname'];//上级
...
// version: 5.0
 ...
 $i = 0;
 /* while( $i -- ){
 	 if( $this_node && strlen( $this_node ) && $i>$settings['max_user_level']){
 ...
 while( $i ++){
 	 if( $this_node && strlen( $this_node ) && $i>$settings['max_user_level']){
 		 $this_node = jlj5( $this_node, $ppddxx['jb']*floatval($settings['jl_share1']));
 	 }
 }
// added ends
// deleted by skyrim
