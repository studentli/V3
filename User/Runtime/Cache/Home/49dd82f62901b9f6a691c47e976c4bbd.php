...
<html>
	...
	<body style="background: #fff;">
		<header class="mui-bar mui-bar-nav" style="background: #4096ee;">
			...
			<h1 class="mui-title" style="color:#fff;">新闻公告</h1>
...
<div id="topPopover" class="top_list" >
 	    ...
 	    <ul class="news_list">
 	    <?php if(is_array($list)): foreach($list as $key=>$v): ?><li><a href="<?php echo U('/Home/Index/newsPage');?>?id=<?php echo ($v["if_id"]); ?>"><h3><?php echo ($v["if_theme"]); ?><span>2017-10-13</span></h3>
 	    		<p><?php echo (substr($v["if_content"],0,30)); ?></p>
 	    	</a>
 	    	</li><?php endforeach; endif; ?>
 	    </ul>
		 <style>
		 	.news_list{padding:15px;padding-top:50px;height:auto;overflow: hidden;}
		 	.news_list li{width:100%;height:auto;overflow: hidden;border-bottom:1px dashed #ddd;}
		 	.news_list li a{width:100%;height:auto;overflow: hidden;}
		 	.news_list a h3{width:100%;font-size: 16px;line-height: 36px;color:#333;padding-left: 10px;}
		 	.news_list a h3 span{font-size:14px;line-height:36px;float: right;color:#4096ee;}
		 	.news_list a p{text-indent:10px;font-size: 14px;line-height: 24px;}
		 </style>
