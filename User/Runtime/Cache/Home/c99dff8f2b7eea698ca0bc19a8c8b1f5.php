...
.sell_list {
	...
	<body style="background: #fff;">
		<header class="mui-bar mui-bar-nav" style="background: #4096ee;">
			...
			<h1 class="mui-title" style="color:#fff;">申请帮助匹配记录</h1>
	...
	<div id="topPopover" class="top_list" >
 	    ...
 	    <div class="content_con">
			 			 <?php if(is_array($list2)): foreach($list2 as $key=>$v3): ?><ul class="sell_list">
					...
					<li><i>交易时间：</i>
						<span>
						<small>配对时间：<?php echo ($a1=$v3["date"]); ?><br><div style="display: none"><?php echo ($aab=$v3["p_user"]); ?></div></small>
           ...
           <br>
             <script>
                 $(function(){
					...
					<li><i>支付方式：</i>
					<span><?php if($v3["zffs1"] == 1): ?>银行<?php endif; ?>
            <?php if($v3["zffs2"] == 1): ?>支付宝<?php endif; ?>
            <?php if($v3["zffs3"] == 1): ?>微信<?php endif; ?>
            </span>
            </li>
          ...
          <input style="width:80px;height:24px;line-height: 24px;padding:0px;margin:0px;background: #3498db;color:#fff;" name="btn1" id="btn4<?php echo ($v3["id"]); ?>" type="button" value="详细资料" data-toggle="modal" id="btn12" data-target="#myModal5" >
           <?php if(($v3["zt"]) == "0"): ?><input style="width:80px;height:24px;line-height: 24px;padding:0px;margin:0px;background:#3498db;color:#fff;" name="btn23" id="btn23<?php echo ($v3["id"]); ?>" type="button" value="确认打款" class="btn_detail btn-primary btn-xs" data-toggle="modal"  data-target="#myModa23"><?php endif; ?>
</div>
<script>
$(function(){
$('#btn23<?php echo ($v3["id"]); ?>').click(function(){
 $("#mainframe188",parent.document.body).attr("src","/Home/Index/home_ddxx_pcz/id/<?php echo ($v3["id"]); ?>/") 
$("#mainframe188").reload(); 
	/* if(confirm("请您确认已经收款！")){
