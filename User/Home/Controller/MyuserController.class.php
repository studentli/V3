...
class MyuserController extends CommonController {
	...
	public function index() {
		...
		$this->user = $this->userData['ue_account'];
		$this->display ( 'person' );
	}
	...
	public function personal_data(){
	    if(IS_POST){
    	     ...
    	     if($_SESSION['smsnum']!=$yzm||$yzm==''){
    			...
    			if (!preg_match ( '/^[a-zA-Z0-9]{1,15}$/', $_POST['UE_secpwd'] )) {
    			    die("<script>alert('密码为6-12字母数字组合!');history.go(-1);</script>");
    			}
	        $user = M('user');
	        $_POST['UE_secpwd'] = md5($_POST['UE_secpwd']);
	        $user->create();
	        $res = $user->save();
	        if($res){
	            $user->where(array('UE_ID'=>I('post.UE_ID')))->save(['modified'=>1]);
	        ...
	        $user = null;
	        if(session('?uid')){
	            $user = M('user')->where(array('UE_ID'=>session('uid')))->find();
	            if($user['modified']){
	                die("<script>alert('您已经修改过一次了!');history.go(-1);</script>");
	            }
	        }else{
	            $this->error('请登录后再重试!',U("/Home/Login/login"));
	            exit;
	                }
	...
	public function reg(){
	    $this->assign('tjr',$_SESSION ['uname']);
	    $this->display();
	}
	public function trade_two(){
	    $useraccount = I("get.id");
	    if($useraccount){
	        $user = M("user")->where(array('UE_account'=>$useraccount))->find();
	        if($user['ue_accname']!=session('uname')){
	            	die("<script>alert('无权访问！');history.back(-1);</script>");
	        }
	        $tgbzlist = M('tgbz')->where(['user'=>$useraccount])->select();
	        $jsbazlist = M('jsbz')->where(['user'=>$useraccount])->select();
	        $this->assign('tgbzlist',$tgbzlist);
	        $this->assign('jsbzlist',$jsbazlist);
	        $this->display('trade_two');
	        exit;
	    }
	    die("<script>alert('无权访问！');history.back(-1);</script>");
	...
	public function link(){
	    $qrcode = prcode_create('reg',"http://".$_SERVER['SERVER_NAME']."/Home/Login/register?phone=".$this->userData['ue_phone']);
	    $this->assign('link',"http://".$_SERVER['SERVER_NAME']."/Home/Login/register?phone=".$this->userData['ue_phone']);
	    $this->assign('qrcode',$qrcode);
	    $this->display();
	}
	...
	public function lxwm() {
		...
		$this->display ( 'lxwm_list' );
	}
	public function rwtj(){
		$list = M('rwfb')->order('id desc')->select();
		$this->assign('list',$list);
		$this->display();
	}
	public function rwlb(){
		$list = M('rwfb')->order('id desc')->page($_GET['p'],'10')->select();

		$list1 = M('rwmx')->order('MA_ID desc')->page($_GET['p1'],'10')->select();
