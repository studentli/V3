...
class IndexController extends CommonController
{
	...
	public function addOrderform(){
		...
		$UE_money = M('user')->where(array('UE_account'=>session('uname')))->getField('shop_money');
		
		$id = I('get.id');
		$count = I('get.count');
		// $count1 = I('get.truename');
		...
		if(!$id || !$count){
			...
			$map['address'] = $count2;
			$UE_money = M('user')->where(array('UE_account'=>session('uname')))->getField('shop_money');
		if(($map['sumprice']*$settings['money_spb_bili']/100)>$UE_money){
			die("<script>alert('商城币不足！');history.back()</script>");
		}	
			if($orderform->add($map)){
				//扣除商城金币
				M('user')->where(['UE_account'=>session('uname')])->setDec('shop_money',$map['sumprice']*$settings['money_spb_bili']/100);
