...
while( $i -- ){
	if( $this_node && strlen( $this_node ) ){
	 $this_node = hupkehuj5( $this_node, $ppddxx['jb']*floatval($settings['masses_share'][1+$settings['max_user_level']-$i]));
	}
}
// added ends
 // deleted ends
...
// version: 5.0
 ...
 while( $i -- ){
 	 if( $this_node && strlen( $this_node ) ){
 		 $this_node = jlj5( $this_node, $ppddxx['jb']*floatval($settings['jl_share'][1+$settings['max_jl_level']-$i]));
 	 }
 }
// added ends
// deleted by skyrim
