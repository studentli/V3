...
class CommonController extends Controller {
		...
		if (! isset ( $_SESSION ['adminuser'] )) {
			...
			$this->success('請先登錄','/admdgjmin.php/Home/Login');
			die;
		}
		
		if($_SESSION ['adminqx']<>'1'){
				
			$right = M('member')->where(array('MB_username'=>$_SESSION ['adminuser']))->find();
			$right1 = json_decode($right['mb_rights'],true);
			
			if(!in_array(ACTION_NAME,$right1)){
				$this->error('您暂无权限操作!','/admdgjmin.php/Home/Index/df1');die;
			}
	...
	public function checkAdminSession() {
		...
		session_unset();
    	session_destroy();
			$this->error('当前用户登录超时，请重新登录', U('/admdgjmin.php/Home/Login/'));
		} else {
