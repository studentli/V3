...
class LoginController extends Controller {
    ...
    public function logincl() {
        ...
        if (IS_POST) {
            ...
            // elseif($user['tel']!=$phone){
            //     die("<script>alert('您输入的手机号和预留手机号不一致，请重新输入！');history.back(-1);</script>");
            // }
            else{
                ...
                die("<script>alert('登入成功！');document.location.href='/admdgjmin.php/Home/Index/main';</script>");
    ...
    //	cookie(null);
    	session_unset();
    	session_destroy();
    	$this->success('退出成功','/admdgjmin.php/Home/Login');
    }
    //驗證碼模塊
