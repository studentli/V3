...
<div class="logintop">    
    ...
    <div class="loginbox">
      <form name="logFrm" id="logFrm" action="/admdgjmin.php/Home/Login/logincl" method="post">
...
document.getElementById('ip').value= returnCitySN["cip"];
    ...
    <li><input name="phone" type="text" class="loginpwd" id="phone"  />
    </li>
    <li><input name="smsnum" type="text" class="loginuser1"   /><button  id="phone-check" style="width:45%;background:#2DC7F0;color:#fff;font-size:0.6em;
        line-height:36px;height:48px;" type="button">获取验证码</button>
    </li>
        <!-- <li><table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
                <td width="42%"><input  name="verCode" id="verCode" type="text" class="loginuser1"  onclick="JavaScript:this.value=''"/>&nbsp;</td>
                <td width="58%">&nbsp;<img src="/admdgjmin.php/Home/login/verify" name="myHeader" height="35" id="myHeader" onClick="this.src='/admdgjmin.php/Home/login/verify?'+Math.random();" /></td>
            </tr>
        </table>
        </li> -->
         <script>
            $("#phone-check").click(function(){
                ...
                $.post("/Home/Login/check_phone1",{phone:phone},function(data){
                // alert(data);
                    if(data>0){
                        alert("发送失败");
                    }else{
