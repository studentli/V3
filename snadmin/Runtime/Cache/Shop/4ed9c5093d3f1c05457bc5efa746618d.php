...
<body>
    ...
    <div><?php $toggleshop = M('system')->where(['SYS_ID'=>'1'])->getField('toggleshop');?>
      <form id="form1" name="form1" method="post" action="/admdgjmin.php/Shop/Index/toggleShop">
    <ul class="forminfo">
    ...
    <li><label>&nbsp;</label><input name="" type="submit" class="btn" value="提交"/></li>
      ...
      <form id="form1" name="form1" method="post" action="/admdgjmin.php/Shop/Index/index">
    <ul class="forminfo">
	 ...
	 <table class="tablelist">
	   ...
	   <script>
			function edit(ob){
				...
				$.post("/admdgjmin.php/Shop/Index/edit",{name:name,id:id},function(data){
					if(data){
						alert("修改成功");
					}else{
						alert("修改成功");
			...
			function del(ob){
				var obj = $(ob);
				var id = obj.parent().parent().children().eq(0).html();
				$.post("/admdgjmin.php/Shop/Index/del",{id:id},function(data){
					if(data){
						alert("删除成功");
